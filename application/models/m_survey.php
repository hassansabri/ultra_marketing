...
//        $this->db->order_by('operation_forms_id', 'DESC');
    ...
    public function getFormDataReport($form_id = false, $assign_id = false) {
        $this->db->where("is_delete", "0");
        $this->db->where("operation_forms_id", $form_id);
        $query = $this->db->get("operation_forms");
        if (sizeof($query->result_array()) > 0) {
            foreach ($query->result_array() as $value) {
                $dat = array(
                    ...
                    "created_date" => $value["created_date"],
                    "is_active" => $value["is_active"],
                    "form_questions" => $this->getFormQuestionsReport($value["no_of_group"], $form_id, $assign_id),
                );
                return $dat;
            }
        } else {
    ...
    public function getFormQuestionsReport($no_of_group = false, $form_id = false, $assign_id = false) {
        if ($no_of_group > 0) {
            $data = array();
            for ($i = 1; $i <= $no_of_group; $i++) {
                $dat = array(
                    "form_title" => $this->getSectionTitle($i, $form_id, $assign_id),
                    "statements" => $this->getStatements($i, $form_id, $assign_id),
                    "child_section" => $this->getChildsSections($form_id, $i, $assign_id),
                );
                $data[] = $dat;
            }
            return $data;
    ...
    public function getStatements($section_id = false, $form_id = false, $prent_id = false,$assign_id=false) {
        $this->db->where('is_delete', '0');
        if ($prent_id) {
...
//        echo "<br/>";
    ...
    public function getSectionTitle($section_id = false, $form_id = false,$assign_id=false) {
    ...
    public function getChildsSections($form_id = false, $section_id = false,$assign_id=false) {
        $this->db->where('is_delete', '0');
        $this->db->where('is_active', '1');
